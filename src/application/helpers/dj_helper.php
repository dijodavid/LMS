 ...
 * -----------------------------------
 * Basic helper file.
 * All basic helper functions are defined here.
 * 
 * @author Dijo David
 * 
 * -----------------------------------
...
if(!function_exists('add_field'))
{
	function add_field($field, $type= NULL, $label = false)
	{
		...
		{
			$field_html .= "<div class='col-lg-offset-2 col-lg-10 f_error error'>";
...
if(!function_exists('action_field'))
{
	...
	{
		$CI =& get_instance();
		//$on_click = ( !$CI->is_ajax() ) ? 'javascript:history.go(-1);' : 'javascript:closeModal();';
		$on_click = ( !$CI->is_ajax() ) ? 'javascript:window.location.assign(classUrl);' : 'javascript:closeModal();';
		$html =	'<div class="form-group">
					<div class="col-lg-offset-2 col-lg-10 action-field">
						<input type="submit" class="btn btn-primary" value="'.$submit.'" />
						<input type="button" class="btn btn-default cancel" value="'.$cancel.'" onclick="'.$on_click.'" />
					</div>
				 </div>';
		return $html;		 
	}
...
{
	/**
	 * Check entry exists in the table
	 * This is used for members,clubs,categories and items
	 * to enable or disable delete button
	 * 
	 * @param $conditions array of conditions
	 * @return boolean
	 */
	function can_delete($conditions)
	{
		$CI =& get_instance();
		$CI->load->model("transaction");
		return ($CI->transaction->has_entry($conditions)) ? false : true;
	}
}

/* End of file dj_helper.php */
/* Location: ./application/helpers/dj_helper.php */