...
function _alpha_dash_space($str_in = '')
{
	...
	function get_bill_period( $date = NULL )
	{
		$this->load->model("bill");
		
		$data = array();
		$bill_day = ($this->config->item('bill_date')) - 1; //generated day not considered in the billing period
		
		if($date)
		{
			if($this->bill->is_bill_present($date)) //bill end / generation date is present
			{
				$data['to_date'] = $this->bill->get_bill_end_date($date);
				$from_date = $this->bill->get_bill_start_date($date);
				$data['from_date'] = ($from_date) ? $from_date : date("Y-m-d", strtotime($data['to_date']."+1 day -1 month"));
			}
			else //no bill, so create a period based on bill_date in config
			{
				$bill_month = date("F Y", strtotime($date));
				$new_bill_date = date("Y-m-d", strtotime($bill_day." ".$bill_month));
				
				$data['from_date'] = date("Y-m-d", strtotime($new_bill_date."+1 day -1 month"));
				$data['to_date'] = $new_bill_date;
			}
		}
		else //bill period based on current date
		{
			$bill_month = date("F Y");
			$bill_date = date("Y-m-d", strtotime($bill_day." ".$bill_month));
			$data['to_date'] = $bill_date;
			$from_date = $this->bill->get_bill_start_date($bill_date);
			$data['from_date'] = ($from_date) ? $from_date : date("Y-m-d", strtotime($bill_date."+1 day -1 month"));
		}
